<template>
    <transition name="scale-down" mode="out-in">
        ...
        <div v-if="is_loading" key="loading" class="content-banner-skeleton" ref="contentBannerSkeletonContainerRef">
            ...
            <Skeleton bg="#e4e4e4" style="display:flex;" :width="skeleton_width + 'px'" :height="skeleton_height + 'px'"
                animated /><!-- 图片占位 -->
        ...
        <div v-else class="carousel-container">
            ...
            <div class="carousel-content" @mouseover="pauseTimer" @mouseout="resumeTimer">
                <div class="carousel-inner"
                    :style="{'transform': `translateX(-${data.active_carousel_index * 100}%)` }">
                    <div class="item" v-for="(item,index) in data.list">
                        <img :src="item.carousel_img">
                    </div>
                </div>
            </div>
            <div class="vui-carousel" ref='dotsList'
                :style="{'--b-color':data.list[data.active_carousel_index].vui_carousel_color}">
                ...
                <div class="r-box">
                    ...
                    <ul class="dots">
                        <li :class="[index==data.active_carousel_index?'pacman':'dot',data.is_prev?'l':'']"
                            v-for="(item,index) in data.list" @click="jumpToSlide(index)">
                            <!-- 吃豆人 -->
