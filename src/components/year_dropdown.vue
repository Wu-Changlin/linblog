<template>
    <div class="select-container" ref="dropdownRef">
        <div class="select-content">
            <div class="year-dropdown dropdown  contribution-year-active" @click="clickSelect()">
            ...
            <div class="select-list" :style="{display:data.is_show_select?'block':'none'}">
                <!-- 下拉框列表 -->
                <ul>
                    <li :class="{'contribution-year-active':data.active_contribution_year==item.contribution_year?'true':''}"
                        v-for="(item,index) in data.list" :key="item.value"
