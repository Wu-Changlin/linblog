...
updateCurrentActiveTagIdFunction(current_active_tag_id.value);
        ...
        for (let i = 0; i < article_count; i++) {
          frontend_article_list_data.value[i]['title'] = `总页数:${total_pages.value},第${current_page.value}页：` + frontend_article_list_data.value[i]['title'];
        }
    ...
    $postDta('/data/frontend/active_tag_next_page_data.json', { tag_id: current_active_tag_id, tag_name: current_active_tag_name, page: current_page.value })
      .then(response => {
        ...
        if (!is_next_page_loading.value) {
          ...
          let frontend_article_list_data_next = response.article_list_data_next; // 博文列表
          // current_page.value= response.current_page; //当前页数  

          //模拟多次搜索返回随机数量
          let sliced_start = Math.floor(Math.random() * 5);
          const data_count = frontend_article_list_data_next.length;
          let sliced_array = frontend_article_list_data_next.slice(sliced_start, data_count);
          ...
          // console.log('原：', JSON.stringify(frontend_article_list_data.value));
          //数组元素合并
          frontend_article_list_data.value = [...frontend_article_list_data.value, ...sliced_array]
          // console.log('合并：', JSON.stringify(frontend_article_list_data.value));
