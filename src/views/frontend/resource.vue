<template>
  ...
  <div class="feeds-page" @scroll="layoutPageOnReachBottom">
    ...
    <div style="margin-top: 72px;">
      <ContentTag :parentPageTagData="resource_tag_data" :currentActiveTagName="current_active_tag_name"
    ...
    <div class="feeds-container">
      ...
      <Waterfall :parentPageArticleListData="resource_article_list_data" :isLoading="is_loading"
  ...
  function getResourcePageData() {
    ...
    resourceModuleApi.getResourcePageData({})
      .then(response => {
        ...
        resource_article_list_data.value = resource_article_list_data.value.slice(sliced_start, data_count);
        let article_count = data_count - sliced_start; // 博文数量

        for (let i = 0; i < article_count; i++) {
          resource_article_list_data.value[i]['title'] = `总页数:${total_pages.value},第${current_page.value}页：` + resource_article_list_data.value[i]['title'];
        }

        //如果博文列表没有数据，那么显示没有数据占位  
        if (!resource_article_list_data.value) {
  ...
  function getChildClickTag(active_tag_id, active_tag_name) {
    ...
    resourceModuleApi.getChildClickTag({ tag_id: active_tag_id, tag_name: active_tag_name })
      .then(response => {
        ...
        //标题添加选中标签名
        for (let i = 0; i < article_count; i++) {
          resource_article_list_data.value[i]['title'] = `点击${current_active_tag_name.value}，总页数:${total_pages.value},第${current_page.value}页：` + resource_article_list_data.value[i]['title']
        }
    ...
    resourceModuleApi.getActiveTagNextPageData({ tag_id: current_active_tag_id, tag_name: current_active_tag_name, page: current_page.value })
      .then(response => {
        ...
        if (!is_next_page_loading.value) {
          ...
          let resource_article_list_data_next = response.article_list_data_next; // 博文列表
          // current_page.value= response.current_page; //当前页数  

          //模拟多次搜索返回随机数量
          let sliced_start = Math.floor(Math.random() * 5);
          const data_count = resource_article_list_data_next.length;
          let sliced_array = resource_article_list_data_next.slice(sliced_start, data_count);
          ...
          // console.log('原：', JSON.stringify(resource_article_list_data.value));
          //数组元素合并
          resource_article_list_data.value = [...resource_article_list_data.value, ...sliced_array]
          // console.log('合并：', JSON.stringify(resource_article_list_data.value));
