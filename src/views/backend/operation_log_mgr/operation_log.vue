<template>
    ...
    <div class="feeds-page">
        <div class="feeds-container">
            ...
            <Table v-if="flag" :parentPageTableData="operation_log_data" :parentPagePaginationData="pagination_data"
...
<script setup>
    ...
    import operationLogModuleApi from "@/api/backend/operation_log.js";//api接口
    ...
    function queryInputData() {
        ...
        if (Object.keys(query_data).length > 0) {//Object.keys会转化一个数组，判断非空对象
            ...
            // 获取查询数据
            operationLogModuleApi.queryInputData(query_data)
                .then(response => {
                    operation_log_data.value = response.operation_log_data;
    ...
    //获取数据
    function getOperationLogListPageData() {

        operationLogModuleApi.getOperationLogListPageData(pagination_data)
            .then(response => {
                operation_log_data.value = response.operation_log_data;
                pagination_data.current_page = response.current_page;
                pagination_data.current_page_limit = response.current_page_limit;
                ...
                // console.log('operation_log_data', operation_log_data.value)
    ...
    function getChildPaginationChangeData(article_pagination) {
        ...
        // 获取查询数据
        operationLogModuleApi.getChildPaginationChangeData(payload_data)
            .then(response => {
                operation_log_data.value = response.operation_log_data;
                pagination_data.current_page = response.current_page;
                pagination_data.current_page_limit = response.current_page_limit;
                ...
                if (article_pagination.role) {
                    // console.log('role-operation_log_data.value :',operation_log_data.value,  ",article_pagination.role:",article_pagination.role)
                    // 计算属性，筛选出查询数据和表格数据相等元素的数据
                    let math_role_data = computed(() => {
                        return operation_log_data.value.filter(tag => tag.role === article_pagination.role);
                    })
                    ...
                    //  let math_role = operation_log_data.value.filter(tag => tag.role === 1);
                    // console.log('operation_log_data.value.filter(tag => tag.role === 1):',operation_log_data.value.filter(tag => tag.role === 1));
                    // console.log('math_role :',math_role)

                }

                operation_log_data.value = getRandom(operation_log_data.value, pagination_data.current_page_limit);
                // console.log('getChildPaginationChangeData-operation_log_data.value:',operation_log_data.value);
    ...
    */
    ...
    const cancelListen = listenMsg((msgInfo) => {
        if (msgInfo.type === 'add-tag') {
            ...
            operation_log_data.value.unshift(msgInfo.content);
        } else if (msgInfo.type === 'edit-tag') {
            const i = operation_log_data.value.findIndex((e) => e.tag_id === msgInfo.content.tag_id);
            console.log('msgInfo.content.article_id:', msgInfo.content.article_id, ',operation_log_data.value[i]:', JSON.stringify(operation_log_data.value))

            if (i >= 0) {
                operation_log_data.value[i] = msgInfo.content;
