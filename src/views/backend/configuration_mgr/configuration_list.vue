<template>
    <div class="feeds-page">
        <div class="feeds-container">
            ...
            <Table v-if="flag" :parentPageTableData="configuration_list_data" :parentPagePaginationData="pagination_data"
                ...
                <template #query>
                    <el-form :inline="false" class="flex-form" :model="query_form_data" @keyup.enter="queryInputData()">
                            ...
                            <el-form-item label="配置名称">
                                <el-input v-model="query_form_data.configuration_name" placeholder="搜索标签名">
                                </el-input>
                            </el-form-item>
                            <el-form-item label="表单标签">
                                <el-select  v-model="query_form_data.form_tag_type" placeholder="请选择">
        <el-option v-for="item in options_form_tag_type_data" :key="item.form_tag_type" :label="item.form_tag_title" :value="item.form_tag_type" />
                                </el-select>
                            </el-form-item>
                ...
                <!-- 标签列特殊处理 开始-->
                <template #form_tag_name="scope">
                <el-tag v-if="scope.row.form_tag_name">{{ scope.row.form_tag_name }}</el-tag>
                </template>
                <!--  标签列特殊处理 结束-->
...
//       {
//         "form_tag_type":4,
//         "form_tag_name": "checkbox",
//         "form_tag_title":"复选按钮"
//       },
//       {
//         "form_tag_type":5,
//         "form_tag_name": "select",
//         "form_tag_title":"下拉菜单"
//       }
//     )
    /*操作表格数据 开始*/
    ...
    function queryInputData() {
        ...
        if (Object.keys(query_data).length > 0) {//Object.keys会转化一个数组，判断非空对象
            ...
            // 获取查询数据
            configurationModuleApi.queryInputData(query_data)
                .then(response => {
                    configuration_list_data.value = response.configuration_list_data;
                    ...
                    let math_role_data = computed(() => {
                        return configuration_list_data.value.filter(configuration => configuration.form_tag_type === query_data.form_tag_type);
                    })
                    // console.log('math_role_data:',math_role_data.value);

                    configuration_list_data.value = math_role_data.value;
    ...
    const configuration_list_data = ref();
    ...
    //获取数据
    function getConfigurationListPageData() {

        configurationModuleApi.getConfigurationListPageData(pagination_data)
            .then(response => {
                configuration_list_data.value = response.configuration_list_data;
                pagination_data.current_page = response.current_page;
                pagination_data.current_page_limit = response.current_page_limit;
                ...
                console.log('configuration_list_data', configuration_list_data.value)
    ...
    function getChildPaginationChangeData(article_pagination) {
        ...
        // 获取查询数据
        configurationModuleApi.getChildPaginationChangeData(payload_data)
            .then(response => {
                configuration_list_data.value = response.configuration_list_data;
                pagination_data.current_page = response.current_page;
                pagination_data.current_page_limit = response.current_page_limit;
                ...
                if (article_pagination.role) {
                    // console.log('role-configuration_list_data.value :',configuration_list_data.value,  ",article_pagination.role:",article_pagination.role)
                    // 计算属性，筛选出查询数据和表格数据相等元素的数据
                    let math_role_data = computed(() => {
                        return configuration_list_data.value.filter(tag => tag.role === article_pagination.role);
                    })
                    ...
                    configuration_list_data.value = math_role_data.value;
                    //  let math_role = configuration_list_data.value.filter(tag => tag.role === 1);
                    // console.log('configuration_list_data.value.filter(tag => tag.role === 1):',configuration_list_data.value.filter(tag => tag.role === 1));
                    // console.log('math_role :',math_role)

                }

                configuration_list_data.value = getRandom(configuration_list_data.value, pagination_data.current_page_limit);
                // console.log('getChildPaginationChangeData-configuration_list_data.value:',configuration_list_data.value);
...
//获取随机
    ...
    */
    ...
    const cancelListen = listenMsg((msgInfo) => {
        if (msgInfo.type === 'add-configuration') {
            // unshift()方法将一个或多个元素添加到数组的开头，并返回新数组的长度。
            configuration_list_data.value.unshift(msgInfo.content);
        } else if (msgInfo.type === 'edit-configuration') {
            const i = configuration_list_data.value.findIndex((e) => e.configuration_id === msgInfo.content.configuration_id);
            console.log('msgInfo.content.configuration_id:', msgInfo.content.configuration_id, ',configuration_list_data.value[i]:', JSON.stringify(configuration_list_data.value))

            if (i >= 0) {
                configuration_list_data.value[i] = msgInfo.content;
