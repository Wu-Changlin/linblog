...
<script setup>
	...
	function updateCurrentMetaInfoFunction(new_current_meta_info) {
		...
		console.log('updateCurrentMetaInfoFunction:', typeof current_meta_info, ',current_meta_info:', current_meta_info);
	}
	...
	/*	在Vue 3中，onUpdated 生命周期钩子会在组件的更新过程中被调用两次。
		这是因为Vue 会在组件的更新前后各调用一次 onUpdated 钩子。
		如果你在 onUpdated 钩子中执行了一些操作，并且这些操作不应该在组件的初始渲染时执行，你可能会遇到执行两次的问题。
		要解决这个问题，你可以通过比较新旧的依赖来确定是否是首次渲染，或者通过一个标志变量来区分是否是首次进入 onUpdated。
	*/
