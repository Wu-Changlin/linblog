...
const menu = {
    ...
    getPageLayoutData(params) {
        return axiosServiceBackend.post("data/backend/menu_page_layout_data.json", params, { headers: { 'Content-Type': 'application/json' } })
...
throw error; // 重新抛出错误，让外部处理
    ...
    getMenuListPageData(params) {
        return axiosServiceBackend.post("data/backend/menu_list.json", params, { headers: { 'Content-Type': 'application/json' } })
            //是将对象 序列化成URL的形式，以&进行拼接   
            .then(response => {
                // 返回data对象数据中的data数据'data':{"code": 0,"data": [{}]"msg": "xxx"}
                return response.data;
...
throw error; // 重新抛出错误，让外部处理
    ...
    queryInputData(params) {
        return axiosServiceBackend.post("data/backend/menu_query_data.json", params, { headers: { 'Content-Type': 'application/json' } })
            //是将对象 序列化成URL的形式，以&进行拼接   
            .then(response => {
                //返回data对象数据中的data数据'data':{"code": 0,"data": [{}]"msg": "xxx"}
                return response.data;
